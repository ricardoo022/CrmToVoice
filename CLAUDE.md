...
- **Epic 02 (Agent Foundations) — done.** `src/crmToVoice/models/` (`state.py`, `fields.py`, `interpretation.py`) is done (US-AG-01): `AgentState`, per-entity field models (`LeadFields`/`PropertyFields`/`VisitFields`), and the `Interpretation` structured-output schema for the future `interpret_speech` node. `src/crmToVoice/agents/tools/` is also done (US-AG-02): `@tool`-decorated 1:1 passthrough wrappers over the Epic 01 functions, so graph nodes never call `pyairtable`/`airtable/` directly. `src/crmToVoice/config.py` is also done (US-AG-03): `get_openrouter_model()` reads `OPENROUTER_MODEL` from env (no model hardcoded anywhere), and `get_chat_model()` is an `lru_cache`d factory returning a `langchain_openai.ChatOpenAI` pointed at OpenRouter's OpenAI-compatible base URL, keyed by `OPENROUTER_API_KEY` — the single reusable LLM client every future node (`interpret_speech`, and later `read_format_response`) should call instead of building its own. See `docs/backlog/epics/epic-02-agent-foundations.md` and `docs/folder-structure.md`.
...
- `docs/Agent.md` — the LangGraph agent design: session/memory model, the shared `AgentState`, the single-router four-path graph (Router 2/`interpret_speech`, built on LangChain's `create_agent` and bound to read-only search tools, is the *only* router — it absorbs the old session-continuation check itself via the checkpointer's conversation state, and decides for itself when to resolve a Lead/Property mention — no separate deterministic context middleware), the `interrupt()`-based pause/resume mechanism (including the idempotency constraint in §6), error/fallback handling, a node-to-file mapping (§9), a Mermaid diagram of the full graph, and (§8) the runtime decisions: LLM via OpenRouter (model left as a swappable config value), local-only hosting via `langgraph dev` + a thin FastAPI webhook adapter in front of it (no production deploy yet), SQLite checkpointer, and the exact Shortcut↔webhook JSON payload (`{session_id, text}` → `{session_id, reply_text, done}`).
...
- `search_leads`/`search_imoveis` are included even though nothing in this epic calls them — they're for `interpret_speech`/Router 2 (Epic 03) to bind as its own tools and call at its own discretion to resolve name/address mentions, not for a separate deterministic middleware.
...
- **Package layout** (`src/crmToVoice/`, still to build): `graph.py` (StateGraph wiring — the only file that calls `add_node`/`add_edge`/`add_conditional_edges`), `config.py` (LLM settings), `webhook.py` (FastAPI adapter), and `agents/nodes/` (every StateGraph node function: the single router — which also absorbs context lookup — the four intent handlers, the final response node — per-file breakdown not decided yet). `models/` and `agents/tools/` (both done, see above) are not separate installable packages, just subpackages — this repo is a single `pyproject.toml`, not a multi-package monorepo, see `docs/folder-structure.md`.
...
- **Layering**: there is no separate deterministic context-resolution layer, and no separate deterministic session-continuation router either. `interpret_speech`/Router 2 is itself an LLM agent — built on LangChain's `create_agent` — bound to the read-only `search_leads`/`search_imoveis` tools. Because `create_agent` carries conversation state across turns via the checkpointer/thread, telling a fresh turn from a reply to a `pending_question` is just part of what the agent already sees, not a separate pre-check node. It decides for itself, per turn, whether the incoming text mentions a Lead/Property worth looking up, and calls the search tool(s) if so. The agent still never writes to Airtable except at the end of a path (create/update/delete), and never has a write tool bound to it at this step — only reads (search or query).
- **Graph shape**: one router (`interpret_speech`/Router 2 — the single entry point every turn runs through), four paths.
  - `interpret_speech`/Router 2 is the only router now — an earlier draft had a separate deterministic Router 1 just to flag fresh-turn vs. mid-wizard/mid-confirmation; that node is gone, folded into Router 2 as described above. Its own conditional edge reads the `intent` it just produced and routes to one of **Create / Read / Update / Delete**.
