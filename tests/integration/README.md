...
OpenRouter (`OPENROUTER_API_KEY`/`OPENROUTER_MODEL`). Runs in CI too (see `.github/workflows/ci.yml`),
...
| `agents/catalog/interpret_speech/` | Real create/read/update/delete round-trips against the Tag 1 all-tools agent, real OpenRouter + Airtable, cleanup fixture per test. |
| `test_webhook.py` | Real round-trip through `POST /webhook` (in-process `TestClient`, no mocking). |
